...
function ChevronDown() {
  return (
    <svg
      ...
      stroke="#5a5870" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
    >
...
export default function LeadFormModal() {
  ...
  // ── Escape key + body scroll lock (unchanged) ──
  useEffect(() => {
    if (!open) return
  ...
  return (
    <>
      ...
      >
        {/* ── Modal card ── */}
        <div style={{ background: '#111118', border: '1px solid rgba(107,138,255,0.25)', borderRadius: 20, padding: '36px 32px', width: '100%', maxWidth: 460, position: 'relative', boxShadow: '0 24px 64px rgba(0,0,0,0.7)', animation: 'chatIn 0.18s ease' }}>
          ...
          {!submitted ? (
            <>
              ...
              <div style={{ marginBottom: 26 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 6 }}>
                  ...
                  {/* Dot 2 */}
                  <div style={{ width: 26, height: 26, borderRadius: '50%', background: step === 2 ? ACCENT : 'rgba(107,138,255,0.12)', border: step === 2 ? 'none' : '2px solid rgba(107,138,255,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: step === 2 ? '#fff' : '#5a5870', flexShrink: 0, transition: 'all 0.3s ease' }}>
                    2
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', paddingLeft: 4, paddingRight: 4 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: step === 1 ? '#f0eeff' : ACCENT, transition: 'color 0.25s' }}>Contact</span>
                  <span style={{ fontSize: 11, fontWeight: 600, color: step === 2 ? '#f0eeff' : '#5a5870', transition: 'color 0.25s' }}>Qualify</span>
                </div>
              </div>

              {/* ══ Header ══ */}
              <div style={{ marginBottom: 22 }}>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', color: ACCENT, textTransform: 'uppercase', marginBottom: 8 }}>
                  Step {step} of 2
                </div>
                ...
                <p style={{ fontSize: 13, color: '#8b88a8', margin: 0, lineHeight: 1.6 }}>
                  {step === 1
                    ? 'We will show you exactly how Leads Up converts your pipeline.'
                    : 'Help us personalise your experience before the call.'}
                </p>
              </div>
                  ...
                  <div>
                    ...
                    <div style={{ position: 'relative' }}>
                      <select className="lf-select" value={form.businessType} onChange={e => field('businessType', e)} required style={selectStyle} onFocus={focusOn} onBlur={focusOff}>
                        ...
                        {['B2B SaaS', 'Marketing Agency', 'Consulting / Coaching', 'Legal / Financial Services', 'Home Services', 'Recruiting / Staffing', 'Real Estate', 'Other'].map(o => (
                  ...
                  <div>
                    ...
                    <div style={{ position: 'relative' }}>
                      <select className="lf-select" value={form.monthlyLeads} onChange={e => field('monthlyLeads', e)} required style={selectStyle} onFocus={focusOn} onBlur={focusOff}>
                        <option value="" disabled>Select…</option>
                        {['Less than 50', '50–200', '200–500', '500+'].map(o => (
                          <option key={o} value={o}>{o}</option>
                        ))}
                      </select>
                      <ChevronDown />
                  ...
                  <div>
                    ...
                    <div style={{ position: 'relative' }}>
                      <select className="lf-select" value={form.budget} onChange={e => field('budget', e)} required style={selectStyle} onFocus={focusOn} onBlur={focusOff}>
                        <option value="" disabled>Select…</option>
                        {['Under $1,000/mo', '$1,000–$2,500/mo', '$2,500–$5,000/mo', '$5,000+/mo'].map(o => (
                          <option key={o} value={o}>{o}</option>
                        ))}
                      </select>
                      <ChevronDown />
                  ...
                  <div>
                    ...
                    <div style={{ position: 'relative' }}>
                      <select className="lf-select" value={form.timeline} onChange={e => field('timeline', e)} required style={selectStyle} onFocus={focusOn} onBlur={focusOff}>
                        <option value="" disabled>Select…</option>
                        {['Immediately', 'Within 30 days', 'Within 90 days', 'Just exploring'].map(o => (
                          <option key={o} value={o}>{o}</option>
                        ))}
                      </select>
                      <ChevronDown />
            ...
            /* ══ Success screen — exactly as before ══ */
