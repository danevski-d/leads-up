...
const C = {
  ...
  primary:     '#6366F1',
  primaryDark: '#7C3AED',
  primaryGlow: 'rgba(99,102,241,0.55)',
  text:        '#F9FAFB',
  textSub:     '#9CA3AF',
  textMuted:   '#4B5563',
  accent:      '#818CF8',
...
/* ─── Integration data ───────────────────────────────────────── */
// 9 nodes at 40° apart, r=210 from center (450,260), viewBox 900×520
const NODES = [
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-lg rounded-2xl p-7 relative"
        style={{ background: '#0D1117', border: '1px solid #1F2937', boxShadow: `0 0 80px -20px ${C.primaryGlow}` }}>
        <button onClick={onClose} style={{ position:'absolute', top:20, right:20, color:C.textMuted, background:'none', border:'none', cursor:'pointer' }}
        ...
        <p style={{ fontSize:13, color:C.textMuted, marginBottom:24 }}>Tell us about your business to get set up.</p>
        ...
        <p style={{ fontSize:11, color:C.textMuted, marginBottom:18 }}>By continuing you agree to our Terms &amp; Privacy Policy.</p>
...
function IntegrationHub() {
  ...
  const NODE_R = 27   // node circle radius (px in SVG coords → ~54px visual)
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto select-none" style={{ paddingBottom: '54%' }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="absolute inset-0 w-full h-full" style={{ overflow:'visible' }}>
        ...
        {/* Subtle outer rings on center */}
        {[90, 130, 170].map((r, i) => (
          <circle key={r} cx={cx} cy={cy} r={r} fill="none"
            stroke={C.primary} strokeWidth={0.5 - i*0.12} strokeOpacity={0.12 - i*0.03}>
        ...
        {/* Connection lines */}
        {NODES.map(n => (
          <path key={`line-${n.id}`} d={PATHS[n.id]}
            stroke={n.ring} strokeWidth="1" fill="none"
            strokeOpacity="0.2" strokeDasharray="4 5" strokeLinecap="round"/>
        ))}
        ...
        {NODES.map(n => (
          <g key={`d2-${n.id}`} filter="url(#glow-sm)">
            <circle r="2" fill={n.ring} opacity="0.45">
              <animateMotion dur={`${DUR[n.id]}s`} repeatCount="indefinite"
                begin={`${DEL[n.id] + DUR[n.id] * 0.45}s`} path={PATHS[n.id]}/>
            </circle>
          </g>
        ))}
      </svg>

      {/* Node circles — logo only */}
      {NODES.map(n => (
        <div key={`node-${n.id}`} className="absolute" style={{
          ...
          <div style={{
            ...
            boxShadow:`0 0 0 0 transparent, 0 6px 20px rgba(0,0,0,0.5), 0 0 16px -4px ${n.ring}30`,
            padding:10,
            transition:'box-shadow 0.3s',
          }}
          onMouseOver={e => e.currentTarget.style.boxShadow=`0 0 0 2px ${n.ring}50, 0 6px 24px rgba(0,0,0,0.6), 0 0 24px -4px ${n.ring}55`}
          onMouseOut={e => e.currentTarget.style.boxShadow=`0 0 0 0 transparent, 0 6px 20px rgba(0,0,0,0.5), 0 0 16px -4px ${n.ring}30`}
          >
      ...
      <div className="absolute" style={{
        ...
        <div style={{ textAlign:'center' }}>
          <div style={{
            ...
            display:'flex', alignItems:'center', justifyContent:'center',
            boxShadow:`0 0 0 3px rgba(99,102,241,0.2), 0 0 50px -8px ${C.primaryGlow}, 0 0 0 6px rgba(99,102,241,0.06)`,
          }}>
...
function Navbar({ onOpenTrial }) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50"
      style={{ background: `${C.bg}E8`, borderBottom:'1px solid #1F2937', backdropFilter:'blur(20px)' }}>
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center">
          {!logoErr ? (
            <img
              src="/leadsup-logo.jpg"
              alt="Leads Up"
              style={{ width:120, height:'auto', maxHeight:36, objectFit:'contain' }}
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span style={{ fontSize:15, fontWeight:700, color:C.text, letterSpacing:'-0.02em' }}>Leads Up</span>
            </div>
          )}
        </Link>

        <div className="hidden md:flex items-center gap-7 text-sm" style={{ color:C.textMuted }}>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <button onClick={onOpenTrial}
            style={{ fontSize:13, fontWeight:600, color:'white', padding:'9px 20px', borderRadius:12, border:'none', cursor:'pointer', background:`linear-gradient(135deg, ${C.primary}, ${C.primaryDark})`, boxShadow:`0 0 20px -6px ${C.primaryGlow}`, transition:'all 0.15s' }}
            onMouseOver={e=>e.currentTarget.style.opacity='0.9'} onMouseOut={e=>e.currentTarget.style.opacity='1'}>
            Get Started Free
          </button>
        </div>

        <button className="md:hidden" style={{ color:C.textMuted, background:'none', border:'none', cursor:'pointer' }} onClick={()=>setOpen(!open)}>
          {open ? <X size={20}/> : <Menu size={20}/>}
        </button>
      </div>

      {open && (
        <div className="md:hidden px-6 pb-5 pt-3 space-y-3" style={{ borderTop:'1px solid #1F2937' }}>
          {[['#features','Features'],['#how-it-works','How It Works'],['#faq','FAQ']].map(([h,l])=>(
            <a key={h} href={h} className="block py-2 text-sm" style={{ color:C.textMuted }} onClick={()=>setOpen(false)}>{l}</a>
          ))}
          <button onClick={()=>{ setOpen(false); onOpenTrial() }}
            className="block w-full mt-3 py-3 text-center text-sm font-semibold text-white rounded-xl"
            style={{ background:`linear-gradient(135deg, ${C.primary}, ${C.primaryDark})` }}>
            Get Started Free
          </button>
        </div>
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden pt-16">
      <div className="absolute inset-0"
        style={{ background:`radial-gradient(ellipse 90% 65% at 50% -10%, rgba(99,102,241,0.2) 0%, transparent 65%), ${C.bg}` }}/>
      {/* Subtle dot grid */}
      <div className="absolute inset-0 opacity-[0.025]"
        style={{ backgroundImage:'radial-gradient(circle, #818CF8 1px, transparent 1px)', backgroundSize:'40px 40px' }}/>

      <div className="relative z-10 w-full max-w-6xl mx-auto px-6 text-center">
        {/* Badge */}
        <div className="inline-flex items-center gap-2 mb-7 px-4 py-1.5 rounded-full"
          style={{ background:'rgba(99,102,241,0.12)', border:`1px solid rgba(99,102,241,0.3)`, fontSize:11, fontWeight:700, color:C.accent, letterSpacing:'0.08em', textTransform:'uppercase' }}>
          <span style={{ width:6, height:6, borderRadius:'50%', background:C.accent, display:'inline-block', animation:'pulse 2s infinite' }}/>
          AI Revenue System
        </div>

        {/* Headline */}
        <h1 style={{ fontSize:'clamp(2.2rem,5.5vw,3.75rem)', fontWeight:700, lineHeight:1.13, letterSpacing:'-0.02em', marginBottom:20, color:C.text }}>
          Never Miss a Lead.{' '}
        ...
        <p style={{ fontSize:'clamp(0.95rem,1.3vw,1.1rem)', color:C.textSub, maxWidth:520, margin:'0 auto 32px', lineHeight:1.7 }}>
          Every inbound call, form, or message gets an AI response in under 60 seconds —
          qualified and booked directly to your calendar.{' '}
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center mb-16">
          <a href="https://cal.com/leads-up" target="_blank" rel="noopener noreferrer"
            ...
            style={{ padding:'14px 28px', borderRadius:14, background:`linear-gradient(135deg, ${C.primary}, ${C.primaryDark})`, color:'white', fontWeight:600, fontSize:15, textDecoration:'none', boxShadow:`0 0 40px -8px ${C.primaryGlow}`, transition:'opacity 0.15s' }}>
            Book a Demo <ArrowRight size={16} className="group-hover:translate-x-0.5 transition-transform"/>
          </a>
          <button onClick={onOpenTrial}
            className="inline-flex items-center justify-center gap-2"
            style={{ padding:'14px 28px', borderRadius:14, border:`1px solid rgba(255,255,255,0.12)`, color:C.accent, background:'rgba(255,255,255,0.04)', fontWeight:600, fontSize:15, cursor:'pointer', transition:'all 0.15s' }}
            onMouseOver={e=>{e.currentTarget.style.borderColor='rgba(99,102,241,0.5)';e.currentTarget.style.background='rgba(99,102,241,0.08)'}}
    ...
    <section id="problem" className="py-24" style={{ background:C.bgSection }}>
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <p style={{ color:C.textSub, fontSize:15, maxWidth:480, margin:'0 auto' }}>
            Speed is the only metric that matters when a new lead lands.
          </p>
        </div>
        <div className="grid md:grid-cols-3 gap-5">
          {[
            ...
            { stat:'44%', label:'of reps give up after one follow-up',  desc:'Manual outreach doesn\'t scale. Leads need instant replies and 5–7 follow-ups within 48 hours.', accent:'#FBBF24' },
            { stat:'68%', label:'of "lost" leads eventually buy — elsewhere', desc:'Without automated reactivation, those contacts quietly become someone else\'s revenue.', accent:'#34D399' },
          ].map(({ stat, label, desc, accent }) => (
            <div key={stat} className="p-7 rounded-2xl"
...
function Features() {
  const features = [
    ...
    { icon:MessageSquare,label:'Multi-touch Follow-up',  headline:'14-day automated nurture.',            desc:'Leads that don\'t respond enter smart SMS + email sequences, stopping the moment they engage.',               highlight:'SMS + email sequences',         color:'#FBBF24', bg:'rgba(251,191,36,0.1)'  },
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-14">
          ...
          <p style={{ fontSize:15, color:C.textSub, maxWidth:480, margin:'0 auto' }}>
            From the first ring or form to a confirmed appointment — handled entirely by AI.
          </p>
        </div>

        {/* AI Receptionist hero card */}
        <div className="mb-5 p-8 rounded-3xl relative overflow-hidden"
          style={{ background:'linear-gradient(135deg, rgba(124,58,237,0.1), rgba(99,102,241,0.06))', border:'1px solid rgba(124,58,237,0.25)', boxShadow:'0 0 60px -20px rgba(124,58,237,0.2)' }}>
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  ...
                  <div style={{ fontSize:19, fontWeight:700, color:C.text }}>Answers Every Call. Books Every Lead.</div>
                </div>
              </div>
              <p style={{ color:C.textSub, lineHeight:1.7, marginBottom:16, fontSize:14 }}>
                When a prospect calls after hours — or when your team is busy — our AI Receptionist picks up, qualifies the caller in a natural conversation, and books an appointment instantly.
              </p>
              <p style={{ color:C.textSub, lineHeight:1.7, marginBottom:20, fontSize:14 }}>
                <span style={{ color:'#C4B5FD' }}>Not a voicemail. Not a bot menu.</span> A real, intelligent interaction.
              </p>
            ...
            <div className="rounded-2xl p-5" style={{ background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.06)' }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <div style={{ marginLeft:'auto', fontSize:10, fontWeight:700, color:C.green }}>Answered in 1s</div>
              </div>
              {[
                {from:'AI',   msg:'Hi, thanks for calling! I\'m the AI assistant. How can I help you today?'},
                {from:'Lead', msg:'Hi, I need my AC serviced. Is someone available this week?'},
                {from:'AI',   msg:'Absolutely! We have Thursday and Friday open. What\'s your address?'},
                ...
                <div key={i} style={{ marginBottom:10, textAlign:from==='Lead'?'right':'left' }}>
                  <div style={{ display:'inline-block', padding:'8px 12px', borderRadius:12, fontSize:11, maxWidth:'88%',
                    background:from==='AI'?'rgba(124,58,237,0.14)':'rgba(255,255,255,0.06)',
                    color:from==='AI'?'#C4B5FD':'#9CA3AF',
                    border:`1px solid ${from==='AI'?'rgba(124,58,237,0.2)':'rgba(255,255,255,0.07)'}`,
                  }}>
                    {msg}
                  </div>
                </div>
    ...
    <section id="how-it-works" className="py-24" style={{ background:C.bgSection }}>
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-14">
          ...
          <p style={{ fontSize:15, color:C.textSub, maxWidth:440, margin:'0 auto' }}>
            Set up in one afternoon. Revenue starts flowing the same day.
          </p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {steps.map(({n,icon:Icon,title,desc},i)=>(
            <div key={n} className="relative">
              {i < steps.length-1 && (
                <div className="hidden lg:block absolute top-6 left-full z-0"
                  style={{ background:`linear-gradient(90deg, rgba(99,102,241,0.3), transparent)`, width:'calc(100% - 20px)', height:1, marginLeft:20 }}/>
              )}
        ...
        <div className="mt-8 p-6 rounded-2xl flex flex-wrap items-center gap-8 justify-between"
        ...
        <div className="space-y-2">
          {items.map(({q,a},i)=>(
            <div key={i} style={{ borderRadius:16, overflow:'hidden', background:open===i?`rgba(99,102,241,0.06)`:C.card, border:`1px solid ${open===i?'rgba(99,102,241,0.25)':C.cardBorder}`, transition:'all 0.2s' }}>
              <button className="w-full text-left" style={{ padding:'18px 24px', display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:16, background:'none', border:'none', cursor:'pointer' }}
                onClick={()=>setOpen(open===i?null:i)}>
                <span style={{ fontSize:13, fontWeight:600, color:C.text, textAlign:'left' }}>{q}</span>
                {open===i
                  ? <ChevronUp size={16} style={{ color:C.accent, flexShrink:0, marginTop:2 }}/>
                  : <ChevronDown size={16} style={{ color:C.textMuted, flexShrink:0, marginTop:2 }}/>}
              </button>
              {open===i && (
                <div style={{ padding:'0 24px 18px', paddingTop:14, fontSize:13, color:C.textSub, lineHeight:1.7, borderTop:`1px solid ${C.cardBorder}` }}>
                  {a}
                </div>
              )}
            </div>
    ...
    <section className="py-24" style={{ background:C.bgSection }}>
      <div className="max-w-4xl mx-auto px-6 text-center">
        <div className="relative p-12 rounded-3xl overflow-hidden"
          style={{ background:`linear-gradient(135deg, rgba(99,102,241,0.12), rgba(124,58,237,0.07))`, border:`1px solid rgba(99,102,241,0.25)`, boxShadow:`0 0 80px -20px ${C.primaryGlow}` }}>
          <div className="absolute inset-0 opacity-25"
            style={{ background:`radial-gradient(ellipse 60% 50% at 50% 0%, rgba(99,102,241,0.4) 0%, transparent 70%)` }}/>
          <div className="relative z-10">
            ...
            <p style={{ fontSize:15, color:C.textSub, marginBottom:32, maxWidth:440, margin:'0 auto 32px' }}>
              Join 1,200+ service businesses. 14-day free trial. No credit card. Setup in under an hour.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center mb-8">
              <button onClick={onOpenTrial}
                className="group flex items-center justify-center gap-2"
                style={{ padding:'15px 36px', borderRadius:14, background:`linear-gradient(135deg, ${C.primary}, ${C.primaryDark})`, color:'white', fontWeight:700, fontSize:16, border:'none', cursor:'pointer', boxShadow:`0 0 50px -8px ${C.primaryGlow}`, transition:'opacity 0.15s' }}>
                Get Started Free <ArrowRight size={17} className="group-hover:translate-x-0.5 transition-transform"/>
              </button>
              <a href="https://cal.com/leads-up" target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-center gap-2"
                style={{ padding:'15px 36px', borderRadius:14, border:`1px solid rgba(255,255,255,0.14)`, color:C.accent, fontWeight:600, fontSize:16, textDecoration:'none', transition:'all 0.15s' }}>
                Book a Demo
              </a>
            </div>
    ...
    <footer style={{ borderTop:`1px solid ${C.cardBorder}`, background:C.bg, padding:'36px 24px' }}>
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex gap-6" style={{ fontSize:13, color:C.textMuted }}>
          <a href="#" style={{ color:C.textMuted, textDecoration:'none' }} onMouseOver={e=>e.currentTarget.style.color=C.text} onMouseOut={e=>e.currentTarget.style.color=C.textMuted}>Privacy</a>
          <a href="#" style={{ color:C.textMuted, textDecoration:'none' }} onMouseOver={e=>e.currentTarget.style.color=C.text} onMouseOut={e=>e.currentTarget.style.color=C.textMuted}>Terms</a>
          <a href="mailto:support@leadsup.io" style={{ color:C.textMuted, textDecoration:'none' }} onMouseOver={e=>e.currentTarget.style.color=C.text} onMouseOut={e=>e.currentTarget.style.color=C.textMuted}>Support</a>
        </div>
