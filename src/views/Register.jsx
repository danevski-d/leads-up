...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const result = await register({ email, password, companyName, fullName })
      if (result.needsEmailConfirmation) {
        setConfirmEmailSent(true)
        setLoading(false)
      }
      // otherwise navigate happens via the useEffect above once user state updates
    } catch (err) {
        ...
        <div className="p-8 rounded-2xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          {confirmEmailSent ? (
            <>
              ...
              <p className="text-sm mb-1" style={{ color: '#3D4165' }}>
                We sent a confirmation link to <span style={{ color: '#9296C4' }}>{email}</span>.
              </p>
              <p className="text-sm mb-7" style={{ color: '#3D4165' }}>
                Confirm your address, then sign in to finish setting up your workspace.
              </p>
              <Link href="/login" className="w-full inline-flex justify-center py-3.5 rounded-xl font-semibold text-white transition-all"
            ...
            <>
              ...
              <p className="text-sm mb-7" style={{ color: '#3D4165' }}>Create your workspace</p>

              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="text-xs font-medium block mb-1.5" style={{ color: '#5A5E80' }}>Full name</label>
                  <input
                    type="text"
                    value={fullName}
                ...
                <div>
                  <label className="text-xs font-medium block mb-1.5" style={{ color: '#5A5E80' }}>Company name</label>
                  <input
                    type="text"
                    value={companyName}
              ...
              <p className="text-center text-sm mt-6" style={{ color: '#3D4165' }}>
                ...
                <Link href="/login" style={{ color: '#7677F4' }}>Sign in</Link>
              </p>
            </>
          )}
        </div>
